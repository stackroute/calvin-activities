...
function deleteRoute(req, res) {
  ...
  l1rService.checkIfCircleIsPresentinCache(route, (err, result) => {
    ...
    l1rService.deleteRoute(route, (err, result) => {
      if (err) { res.status(404).json({ message: err }); return; }
      if (result) { res.status(200).json({ result }); } else { res.status(404).json({ message: `circle with id ${route.circleId} does not have a route for multiplexer with id ${route.multiplexerId} ` }); }
    });
  });
}
