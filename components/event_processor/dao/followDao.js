...
function addFollow(follower, startedFollowing, callback) {
  ...
  client.execute(query, [follower.circleId.toString(), follower.mailboxId.toString(), startedFollowing],
    (err) => {
      if (err) {
        throw err;
      }
      const query2 =
      ('INSERT INTO circlesFollowedByMailbox (mailboxid, circleid, startedFollowing ) values(?, ?, ? )');
      client.execute(query2, [follower.mailboxId.toString(), follower.circleId.toString(), startedFollowing],
        (err2) => {
          if (err2) {
            throw err2;
          }
